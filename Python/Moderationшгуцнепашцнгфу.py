...
async def whatever_you_want_to_call_it(message):
    ...
    if any(word in msg_content for word in Links):
        ...
        EmbedLinks = discord.Embed(
            title = 'Нарушение (Ссылки)',
            description = f'{message.author.mention} отправил ссылку!',
            timestamp = message.created_at
        )
        await get(message.guild.text_channels, id = 1007712770768240743).send(embed = EmbedLinks)
    else:
        return
